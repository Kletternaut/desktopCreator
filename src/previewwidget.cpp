...
void PreviewWidget::updateIconPreview()
{
    ...
    if (!iconPath.isEmpty()) {
        if (iconPath.toLower().endsWith(".svg")) {
            ...
            if (renderer.isValid()) {
                pix = QPixmap(kIconSize, kIconSize);
        ...
        if (pix.isNull()) {
            // Versuche als Theme-Icon
            const QIcon themeIcon = QIcon::fromTheme(iconPath);
            if (!themeIcon.isNull()) {
                pix = themeIcon.pixmap(kIconSize, kIconSize);
            }
        }
    }
    ...
    if (pix.isNull()) {
        ...
        pix = QIcon::fromTheme("application-x-executable",
              QIcon(":/icons/app-icon.svg")).pixmap(kIconSize, kIconSize);
    }
